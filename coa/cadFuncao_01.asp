...
if strAcao = "A" then
	...
	strSql = ""
	strSql = strSql & " select fu.url,  mo.modulo_id, fu.nome,fu.situacao, fu.ordem from "
	strSql = strSql & "       	modulo mo, funcao fu "
	strSql = strSql & " where mo.modulo_id = fu.modulo_id and fu.funcao_id= " & intFuncao
			...
			<div class="main-page">
				...
				<div class="grids">
					<div class="progressbar-heading grids-heading">
						<h2><%if strAcao = "A" then%>Alterar Função<%else%>Cadastro de Função<%end if%></h2>
					</div>
					<div class="forms-grids">
						<div class="col-md-8">
							<div class="panel panel-widget">
								<div class="my-div">
									<form method="post" class="valida"  id="form01" name="form01" action="cadFuncao_02.asp">
  									    <div class="" id="msg">
											<%call msgRetornoSucesso()%>
											<%call msgRetornoErro()%>
										</div>
										<label for="loja" >Módulo:</label>
										<div class="form-group" id="dvModulo" >
											<select name="modulo" id="modulo" class="form-control" required="true">
												<option value=""></option>
												<%
												strSql = "select * from modulo where situacao = 'A'"
												set objRs = server.createobject("adodb.recordset")
												objRs.open strSql, conexao, 3, 3
												if not objRs.eof then
													do while not objRs.eof
														%>
														<option value="<%=objRs("modulo_id")%>" <%if intModuloId = objRs("modulo_id") then%>selected<%end if%>><%=objRs("nome")%></option>
														<%
														objRs.movenext
													loop
												end if
										...
										<div class="form-group" >
											<input type="text" name="url" value="<%=strUrl%>" id="url" required="true" class="form-control"	/>
										</div>
										...
										<input type="hidden" name="funcao_id" value="<%=intFuncao%>">
