...
strSql = ""
strSql = strSql & " select fu.funcao_id, fu.nome nome_funcao, mo.nome nome_modulo, fu.situacao,  fu.ordem, fu.url, fu.funcao_pai_id, (select fu2.nome from funcao fu2 where fu2.funcao_id=fu.funcao_pai_id) nome_funcao_pai from "
strSql = strSql & "       	funcao fu, modulo mo "
strSql = strSql & " where 1=1 and fu.modulo_id = mo.modulo_id"

if strNome <> "" then
	strSql = strSql & "	 and upper(fu.nome) like '%upper(" &  strNome & ")%' "
end if

if strModuloId <> "" then
	strSql = strSql & "	 and fu.modulo_id = " &  intModuloId
end if

if strSituacao <> "" then
	strSql = strSql & "	  and fu.situacao = '" &  strSituacao & "'"
end if

strSql = strSql & " order by fu.ordem "
									...
									<form method="post" class="valida"  id="form01" name="form01" action="">
										<div class="row" >
											<div class="col-xs-12 col-sm-5" >
												<label><b>Modulo:</b>&nbsp; <%=retornaNomeModulo(intModuloId)%></label>
											</div>
													...
													if not objRs.eof then
														do while not objRs.eof and contador<final
															...
															<tr <%if contador mod 2 then %>class="info" <%end if%>>
																<td><%=objRs("funcao_id")%></td>
																<td><%=objRs("nome_funcao")%></td>
																<td><%=objRs("url")%></td>
																<td><%=objRs("nome_funcao_pai")%></td>
																<td><%=objRs("ordem")%></td>
																<td>
																	<select name="situacaoFuncao" id="situacaofuncao" class="form-control" onchange="javascript:atualizaSituacao(this.value);" >
																		<option value="A|<%=objRs("funcao_id")%>" <%if objRs("situacao") = "A" then response.write "selected"%>>Ativo</option>
																		<option value="I|<%=objRs("funcao_id")%>" <%if objRs("situacao") = "I" then response.write "selected"%>>Inativo</option>
																	</select>
																</td>
																<%if alterar = "S" then%>
																<td align="center"><button type="button" class="btn btn-info" onclick="javascript:alterar('<%=objRs("funcao_id")%>')">Alterar</button></td>
																<%end if%>
															</tr>
															<%
															end if
															...
															<nav>
															  <ul class="pagination">
																...
																For i=1 to totalRegistroPagina
																	If i=pagina Then
																		response.Write "<li class='active'><a href=javascript:paginar('consFuncao_02.asp?pagina="&i&"','" & pagina &"')> "&i&"<span class='sr-only'>(current)</span></a></li>"
																	Else
																		response.Write "<li><a href=javascript:paginar('consFuncao_02.asp?pagina="&i&"','" & pagina& "')> "&i&" </a></li>"
																	End if
																Next
																%>
																<%If totalRegistroPagina>pagina Then%><li><a href="javascript:paginar('consFuncao_02.asp?pagina=<%=(pagina+1)%>','<%=(pagina+1)%>')" aria-label="Next"><i class="fa fa-angle-right"></i></a></li><%end if%>
															 </ul>
														   </nav>
														</td>
													</tr>
		...
		<script type="text/javascript" >
			...
			$(document).ready( function(){
				...
				$('#cadastrar').click( function(){
					$('form').attr('action', 'cadFuncao_01.asp');
					$('form').trigger( 'submit' );
				});
				
				$('#voltar').click( function(){
					$('form').attr('action', 'consFuncao_01.asp');
					$('form').trigger( 'submit' );
				});
